...
namespace sTASKedit
{
	public class AWARD_DATA
	{
		...
		public int m_iSoloTowerChallengeScore;
		...
		public bool AwardLeaveCarrier;

		public byte[] m_CandItemsPointer, m_MonstersPointer, m_RankingAwardPointer, m_ulChangeKeyPointer,
		m_plHistoryChangeKeyPointer, m_ulDisplayKeyCntPointer, m_pszExpPointer, m_pTaskCharPointer, m_pTitleAwardPointer;
		...
		internal static AWARD_DATA Read(int version, BinaryReader br)
		{
			...
			if (version >= 124)
			{
				reward.m_iSoloTowerChallengeScore = br.ReadInt32();
			}
			else
			{
				reward.m_iSoloTowerChallengeScore = 0;
			}
			if (version >= 75)
			{
			...
			if (version >= 112)
			{
				...
				reward.m_plHistoryChangeKeyPointer = br.ReadBytes(12);
			}
			else
			{
				reward.m_ulHistoryChangeCnt = 0;
				reward.m_plHistoryChangeKeyPointer = new byte[12];
			}
			if (version >= 59)
			{
			...
			if (version >= 101)
			{
				reward.m_iTaskLimit = br.ReadInt32();
			}
			else
			{
				reward.m_iTaskLimit = 0;
			}
			if (version >= 109)
			{
			...
			if (version >= 115)
			{
				reward.m_iLeaderShip = br.ReadInt32();
			}
			else
			{
				reward.m_iLeaderShip = 0;
			}
			if (version >= 119)
			{
				reward.m_iWorldContribution = br.ReadInt32();
			}
			else
			{
				reward.m_iWorldContribution = 0;
			}
			if (version >= 127)
			{
			...
			if (version >= 135)
			{
				reward.AwardLeaveCarrier = br.ReadBoolean();
			}
			else
			{
				reward.AwardLeaveCarrier = false;
			}
			reward.m_CandItems = new AWARD_ITEMS_CAND[reward.m_ulCandItems];
			for (int index = 0; index < reward.m_ulCandItems; index++)
			{
			...
			if (version >= 109)
			{
				...
				for (int index8 = 0; index8 < reward.m_ulTitleNum; index8++)
				{
		...
		internal static void Write(int version, BinaryWriter bw, AWARD_DATA reward)
		{
			...
			if (version >= 124)
			{
				bw.Write(reward.m_iSoloTowerChallengeScore);
			}
			if (version >= 75)
			{
			...
			if (version >= 119)
			{
				bw.Write(reward.m_iWorldContribution);
			}
			if (version >= 127)
			{
			...
			if (version >= 135)
			{
				bw.Write(reward.AwardLeaveCarrier);
			}
			for (int index = 0; index < reward.m_ulCandItems; index++)
			{
